...
/*   Updated: 2018/06/28 23:19:21 by mrodrigu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_map		*ft_destroy_path(t_map **map)
{
	...
	tmp = (*map)->prev ? tmp : NULL;
	tmp2 = (*map)->prev ? tmp2 : NULL;
	free((*map));
	(*map) = tmp2;
	return (tmp);
}
...
t_map		*ft_prepare_graf(t_node *node, t_map *map, int pos, int mode)
{
	...
	while (node)
	{
		if (node->start)
			node->ihbt = 0;
		else if ((node->ihbt && !mode && (node->ihbt != -1 ||
					compare_path(node, path))) || node->end)
			node->ihbt = 0;
						...
						int *cuant, t_map **conj)
{
	...
	tmp = 0;
	if (!(*cuant - 1) || tail_head[1]->path->next->node->id !=
							tail_head[1]->prev->path->next->node->id)
	{
		tmp = tail_head[1]->len;
	}
	tmp < 0 ? tmp *= -1 : tmp;
